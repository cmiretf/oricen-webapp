<template>
  <footer class="bg-white border-t">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div class="md:col-span-1">
          <div class="flex flex-col items-start">
            <img :src="logo" alt="Oricen Logo" class="h-16 w-auto mb-4" />
            <div class="flex gap-3 mt-4">
              <a href="https://www.linkedin.com/company/talenthia-solutions" target="_blank" rel="noopener noreferrer" class="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-[#2B4C7E] hover:text-white transition-colors">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
              </a>
              <a href="https://www.instagram.com/_oricen_/?igsh=MXExaGl0OWVsZHM0#" target="_blank" rel="noopener noreferrer" class="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-[#2B4C7E] hover:text-white transition-colors">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a href="https://www.tiktok.com/@_oricen_?_t=ZN-90vflCEY6tS&_r=1" target="_blank" rel="noopener noreferrer" class="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-[#2B4C7E] hover:text-white transition-colors">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>

        <div>
          <p class="text-sm text-gray-600">info@oricen.com</p>
          <p class="text-sm text-gray-600 mt-2">+34 633580559</p>
          <p class="text-sm text-gray-600 mt-4">
            <a href="https://maps.app.goo.gl/PD75dBfY14otDTfP7" target="_blank" rel="noopener noreferrer" class="text-[#2B4C7E] underline hover:no-underline">
              Camino Valdenigriales<br/>
              S/N, 28223, Pozuelo de<br/>
              Alarcón (ESIC University)
            </a>
          </p>
        </div>

        <div>
          <nav class="flex flex-col gap-2">
            <router-link to="/politica-privacidad" class="text-sm text-[#2B4C7E] underline hover:no-underline">Política de Privacidad</router-link>
            <a href="#" class="text-sm text-[#2B4C7E] underline hover:no-underline">Declaración de Accesibilidad</a>
            <a href="#" class="text-sm text-[#2B4C7E] underline hover:no-underline">Términos y Condiciones</a>
          </nav>
        </div>

        <div>
          <h4 class="font-bold text-gray-900 mb-4">Mantente informado con nosotros!</h4>
          <form class="space-y-3">
            <div>
              <label class="text-sm text-gray-600">Email *</label>
              <input
                type="email"
                class="w-full mt-1 px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#2B4C7E]"
              />
            </div>
            <label class="flex items-start gap-2 text-sm text-gray-600">
              <input type="checkbox" class="mt-1" />
              <span>Sí, quiero suscribirme a la newsletter. *</span>
            </label>
            <button type="submit" class="w-full btn-primary">
              Subscribe
            </button>
          </form>
        </div>
      </div>

      <div class="mt-12 pt-8 border-t">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6">
          <div class="text-center md:text-left flex-1">
            <p class="text-sm font-semibold text-gray-700 mb-2">UNIVERSO START<span class="text-[#C4D70F]">up</span></p>
          </div>
          
          <div class="flex items-center gap-4">
            <div class="w-px h-8 bg-[#2B4C7E]"></div>
            <p class="text-sm font-semibold text-[#2B4C7E]">COLABORADORES</p>
            <div class="w-px h-8 bg-[#2B4C7E]"></div>
          </div>
          
          <div class="text-center md:text-right flex-1">
            <p class="text-sm font-semibold text-[#2B4C7E]">ESIC<br/>University</p>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
import logo from '../assets/logo.avif'
</script>
